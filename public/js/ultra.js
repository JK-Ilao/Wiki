    ...
    .then(data => {
      ...
      data.forEach(row => {
          ...
          if (row.Time) {
            ...
            timeElement.innerHTML = `Time: <a href="/Beast?type=Time&value=${row.Time}">${row.Time}</a>`;
          ...
          if (row.Condition) {
            ...
            conditionElement.innerHTML = `Condition: ${row.Condition === '-' ? 'None' : `<a href="/Beast?type=Condition&value=${row.Condition}">${row.Condition}</a>`}`;
